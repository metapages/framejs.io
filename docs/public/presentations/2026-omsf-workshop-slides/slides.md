...
<span class="kicker">OMSF Workshop · 2026</span>
...
- **AI integration** — describe it, an agent builds it <span class="muted">(the
  main event)</span>
...
- The whole app is **encoded in the URL**. The URL _is_ the program.
...
## AI integration <span class="muted">— the main event</span>
...
<div class="url-card live">
  <span class="state">Work in progress</span>
  <p class="url">framejs.io/<span class="var">#&lt;your code&gt;</span></p>
  <ul>
    <li>Code lives in the <strong>hash</strong></li>
    <li>Never sent to a server</li>
  </ul>
</div>

<div class="url-card">
  <span class="state">Saved &amp; shareable</span>
...
<p class="concl">Same app, two URLs — one private and live, one fixed and shareable. <span class="muted">framejs.io/docs</span></p>
...
- Every iteration mints a **new immutable URL** — the old one stays intact
- Chain as many follow-ups as you like: layout, colours, filters, annotations

<p class="concl">The URL is the save point, you iteration just by prompting.</p>
...
## Advanced usage: collaborative design

Run a meeting, capture what the team actually wants, then turn that into a
visualization — without a designer in the loop.

<ol class="steps">
  <li><strong>Have the discussion</strong> — talk through what the chart should show, what questions it needs to answer, what the audience cares about</li>
  <li><strong>Save the transcript</strong> — meeting notes, a recorded transcript, or just a rough summary</li>
  <li><strong>Add the raw data</strong> and any reference examples or sketches</li>
  <li><strong>Prompt the agent</strong> — it reads the discussion, understands the intent, and builds the visualization incorporating what was decided</li>
</ol>

<p class="concl">The human conversation <em>becomes</em> the spec, AI turns it into the visualization in minutes.</p>

Note: This is the use case that surprises people most. A meeting or Slack thread
already contains the design intent — what metric matters, what comparison the
PI wants to see, what the reviewer found confusing. Feed that context to the
agent alongside the data and you get a first version that reflects real
decisions, not just defaults. Iterate from there.
...
## Data storage policy and direction

- State lives in the **URL hash** — the part after `#` the browser **never sends
  to the server**.
- **No server-side database** of your work. _You_ hold it, in the link.
- **Short URLs**: the server drops your code in a content-addressed bucket and
  returns a hash. It **never executes or inspects** your code — a dumb store, on
  purpose.
- Short URLs are **immutable** but expire in **1 month**. User accounts for permanent storage coming soon<sup class="fn">†</sup>.

<p class="concl">Self-contained → shareable. Self-contained → private. The same property, both directions.</p>

<br/>
<br/>
<br/>
<p class="footnote"><sup>†</sup> "Don't expire" is the intent, not a hard guarantee — retention, storage costs, and what permanence really means operationally are a longer discussion.</p>

Note: This matters especially for science. Your code and state are in the URL
hash, which the browser does not transmit. There's no backend holding a copy
with your name on it. When you want a tidier link, the shortener stores the
bundle in object storage — content-addressed, immutable, no expiry — and never
runs or reads it. People assume private and shareable are in tension; they're
not. A fully self-contained thing is both at once. If you don't share it, nobody
has it.
